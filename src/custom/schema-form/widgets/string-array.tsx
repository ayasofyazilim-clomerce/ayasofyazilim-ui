...
import { InputGroup, InputGroupAddon, InputGroupButton, InputGroupInput, InputGroupText } from "@repo/ayasofyazilim-ui/components/input-group";
import { Tooltip, TooltipContent, TooltipTrigger } from "@repo/ayasofyazilim-ui/components/tooltip";
...
interface StringArrayWidgetConfig {
    ...
    renderItem?: (value: string, index: number, onRemove: () => void) => React.ReactNode;
...
export function CustomStringArrayWidget(config: StringArrayWidgetConfig = {}) {
    ...
    const Widget = (props: WidgetProps) => {
        ...
        return (
            <InputGroup className={cn("p-1.5", currentValues.length === 0 && "p-0", classNames.container)}>
                <InputGroupAddon
                    align="block-end"
                    className={cn("flex-col gap-1.5", maxHeight, "overflow-auto p-0", classNames.listContainer || "")}
                >
                    {currentValues.length === 0 && emptyState ? (
                        ...
                        currentValues.map((value: string, index: number) => {
                            ...
                            return (
                                <InputGroupText
                                    className={cn("bg-accent relative w-full overflow-hidden pl-2 pr-1 min-h-8 h-8 rounded-md", classNames.listItem || "")}
                                    key={`${value}-${index}`}
                                >
                                    <span className={cn("truncate text-ellipsis", classNames.listItemText || "")}>
                                        {value}
                                    </span>
                                    <InputGroupButton
                                        className={cn("rounded-md ml-auto", classNames.removeButton || "")}
                ...
                <InputGroupAddon align="block-end"
                    className={cn("p-0 justify-between", currentValues.length === 0 && "pr-1.5 ", classNames.inputContainer || "")}>
                    <Tooltip open={!!error}>
                        <TooltipTrigger asChild>
                            <div>
                                <InputGroupInput
                                    className={cn("pl-2", error && "text-destructive", classNames.input || "")}
                        ...
                        <TooltipTrigger asChild>
                            <div>
                                <InputGroupButton
                                    ...
                                    className={cn("rounded-md ml-auto", classNames.addButton || "")}
