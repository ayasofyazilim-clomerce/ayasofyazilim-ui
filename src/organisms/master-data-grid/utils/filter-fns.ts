...
export const masterFilter: FilterFn<unknown> = (row, columnId, filterValue) => {
    ...
    switch (operator) {
        // String operators
        case 'equals':
            return toString(cellValue).toLowerCase() === toString(value).toLowerCase();

        case 'notEquals':
            return toString(cellValue).toLowerCase() !== toString(value).toLowerCase();

        case 'contains':
            return toString(cellValue).toLowerCase().includes(toString(value).toLowerCase());

        case 'notContains':
            return !toString(cellValue).toLowerCase().includes(toString(value).toLowerCase());

        case 'startsWith':
            return toString(cellValue).toLowerCase().startsWith(toString(value).toLowerCase());

        case 'endsWith':
            return toString(cellValue).toLowerCase().endsWith(toString(value).toLowerCase());
        ...
        case 'before': {
            ...
            return dateValue !== null && dateFilter !== null && dateValue < dateFilter;
        }
...
export function getFilterOperators(
    ...
    // Enum operators
    if (format === 'enum') {
        return ['equals', 'notEquals', 'inList', 'notInList', 'isEmpty', 'isNotEmpty'];
    }
