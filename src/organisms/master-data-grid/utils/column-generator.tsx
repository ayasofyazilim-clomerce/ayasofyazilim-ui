...
import type { ColumnConfig, CustomRenderers, GeneratedColumn, JSONSchema, JSONSchemaProperty, Localization } from '../types';
...
export function generateColumnsFromSchema<TData = unknown>(
    ...
    Object.entries(schema.properties).forEach(([key, property]) => {
        const column = createColumnFromProperty<TData>(key, property as JSONSchemaProperty, localization, t, onFilterClick, editingContext, cellClassName, dateOptions, customRenderers, errorDisplayMode, enableColumnVisibility);
        if (column) {
            columns.push(column);
        }
    });
...
function createColumnFromProperty<TData = unknown>(
    ...
    return {
        ...
        cell: ({ getValue, row }: { getValue: () => unknown; row: { original: TData; index: number }; column: Column<TData> }) => {
            const rowId = editingContext?.getRowId(row.original, row.index) || String(row.index);
            const isEditing = editingContext?.editingRows[rowId] !== undefined;
            const editedValue = editingContext?.editingRows[rowId]?.[key];
            const displayValue = isEditing && editedValue !== undefined ? editedValue : getValue();
            ...
            return (
                <CellRenderer
                    ...
                    t={t}
                    editable={isEditing}
                    onUpdate={isEditing ? (value) => editingContext?.onCellUpdate(rowId, key, value) : undefined}
                    className={className}
...
export function mergeColumns<TData = unknown>(
    ...
    schemaColumns.forEach(schemaCol => {
        ...
        if (custom) {
            ...
            if (customCell && schemaCell && typeof schemaCell === 'function' && editingContext) {
                finalCell = (props) => {
                    // Check if this row is in edit mode
                    const rowId = editingContext.getRowId(props.row.original, props.row.index) || String(props.row.index);
                    const isEditing = editingContext.editingRows[rowId] !== undefined;

                    // In edit mode, use schema-based cell (editable inputs)
                    if (isEditing) {
                        return typeof schemaCell === 'function' ? schemaCell(props) : schemaCell;
                    }

                    // Otherwise, use custom cell renderer
                    return typeof customCell === 'function' ? customCell(props) : customCell;
                };
            }
...
export function calculateColumnWidth(
    ...
    if (property.enum && property.enum.length > 0) {
        const maxLength = Math.max(...property.enum.map((v: string | number) => String(v).length));
