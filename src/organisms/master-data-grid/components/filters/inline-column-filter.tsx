 ...
 */
export function InlineColumnFilter<TData>({ column, t }: InlineColumnFilterProps<TData>) {
    ...
    const [operator, setOperator] = useState<FilterOperator>(currentFilter?.operator || availableOperators[0] || 'contains');
                ...
                <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                        <button className="w-full px-2 py-1.5 text-xs font-medium hover:bg-accent rounded-md flex items-center justify-between">
                            {getTranslations(`filter.operator.short.${operator}`, t)} <ChevronDown className="size-3" />
                        </button>
                    </DropdownMenuTrigger>
