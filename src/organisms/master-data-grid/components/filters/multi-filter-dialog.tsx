...
export function MultiFilterDialog<TData>({
    ...
    const [filterRows, setFilterRows] = useState<FilterRow[]>(() => {
        ...
        return [{ id: 'filter-0', columnId: '', operator: 'contains', value: '', value2: '' }];
    });
    ...
    const addFilter = () => {
        ...
        setFilterRows([...filterRows, { id: newId, columnId: '', operator: 'contains', value: '', value2: '' }]);
    };

    const removeFilter = (id: string) => {
        if (filterRows.length === 1) {
            // Reset to empty instead of removing last one
            setFilterRows([{ id: 'filter-0', columnId: '', operator: 'contains', value: '', value2: '' }]);
        } else {
    ...
    const applyFilters = (closePopover?: () => void) => {
        ...
        filterRows.forEach(row => {
            ...
            const needsNoInput = row.operator === 'isEmpty' || row.operator === 'isNotEmpty';
        ...
        <div className="space-y-4">
                ...
                {filterRows.map((row, index) => {
                    ...
                    return (
                        <ButtonGroup className='w-full min-w-0 flex-wrap sm:flex-nowrap'>
                            ...
                            <Select value={row.columnId} onValueChange={(value) => handleColumnChange(row.id, value)}>
                                <SelectTrigger className="w-32 sm:w-40 min-w-0">
                                    <SelectValue placeholder={getTranslations('filter.selectColumn', t)} />
                                </SelectTrigger>
                                <SelectContent>
                            ...
                            <Select
                                value={row.operator}
                                onValueChange={(value) => updateFilter(row.id, { operator: value as FilterOperator })}
                                disabled={!row.columnId}>
                                <SelectTrigger className="w-16 sm:w-40 min-w-0">
                                    <span className="flex items-center gap-2">
                                        <span className="sm:hidden">{getOperatorIcon(row.operator)}</span>
                                        <span className="hidden sm:inline">{getTranslations(`filter.operator.${row.operator}`, t)}</span>
                                    </span>
                                </SelectTrigger>
                                <SelectContent>
                                    {availableOps.map(op => (
                                        <SelectItem key={op} value={op}>
                                            <span className="flex items-center gap-2">
                                                {getOperatorIcon(op)}
                                                <span>{getTranslations(`filter.operator.${op}`, t)}</span>
                                            </span>
                                        </SelectItem>
                                    ))}
                                </SelectContent>
                            ...
                            <FilterInput
                                ...
                                onSliderChange={(values) => {
                                    ...
                                    updateFilter(row.id, { value: String(min), value2: String(max) });
                                }}
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2 pt-4 border-t">
                <div className="flex flex-col sm:flex-row gap-2">
                    <Button onClick={addFilter} variant="outline" size="sm" className="text-xs sm:text-sm">
