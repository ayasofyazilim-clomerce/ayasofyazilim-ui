...
export function FilterInput({
    ...
    const isNumberType = schemaProperty?.type === 'number' || schemaProperty?.type === 'integer';
    ...
    const sliderMin = useMemo(() => schemaProperty?.minimum ?? 0, [schemaProperty?.minimum]);
    const sliderMax = useMemo(() => schemaProperty?.maximum ?? 100, [schemaProperty?.maximum]);
            ...
            return (
                <InputGroup>
                    <Select value={value} onValueChange={onValueChange}>
                        ...
                        <SelectContent>
                            <SelectItem value="true">{getTranslations('filter.true', t)}</SelectItem>
                            <SelectItem value="false">{getTranslations('filter.false', t)}</SelectItem>
                        </SelectContent>
                    </Select>
                    {value && (
                        <InputGroupButton size="icon-xs" onClick={onClear} aria-label={getTranslations('filter.clearFilter', t)}>
                            <X />
                        </InputGroupButton>
                    )}
                </InputGroup>
            ...
            return (
                <div className="space-y-3">
                    <InputGroup className='rounded-none border-0 shadow-none'>
                        ...
                        <InputGroupInput
                            value={value2}
                            onChange={(e: React.ChangeEvent<HTMLInputElement>) => onValue2Change(e.target.value)}
                    ...
                    {showSlider && onSliderChange && (
                        <div className="space-y-2">
                            <Slider
                                ...
                                max={sliderMax}
                                step={1}
                                value={[Number(value) || sliderMin, Number(value2) || sliderMax]}
                                onValueChange={onSliderChange}
                                className="w-full"
                            />
                            <div className="flex justify-between text-xs font-medium text-foreground">
                                <span className="px-2 py-0.5 rounded bg-muted">{value || sliderMin}</span>
                                <span className="px-2 py-0.5 rounded bg-muted">{value2 || sliderMax}</span>
                            </div>
                        </div>
                    )}
                </div>
        ...
        return (
            <InputGroup className='flex-1 min-w-0'>
                ...
                <span className="text-muted-foreground px-0.5 sm:px-1">
                    ...
                    <span className="hidden sm:inline text-xs">{getTranslations('filter.to', t)}</span>
                </span>
                <InputGroupInput
                    placeholder={getTranslations('filter.max', t)}
                    value={value2}
