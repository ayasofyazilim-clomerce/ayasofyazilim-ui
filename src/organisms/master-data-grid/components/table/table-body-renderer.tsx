...
import { getPinningCellClassNames, getPinningCellStyles } from '../../utils/pinning-utils';
                        ...
                        >
                            {row.getVisibleCells().map(cell => {
                                const cellClass = typeof cellClassName === 'function'
                                    ? cellClassName({ row: row.original, columnId: cell.column.id })
                                    : cellClassName;
                                ...
                                return (
                                    <TableCell
                                        ...
                                        className={cn(
                                            ...
                                            editingEnabled && !isEditing && 'hover:border-muted-foreground/40'
                                        )}
                                    >
                        ...
                        {row.getIsExpanded() && expansionEnabled && (
                            <TableRow>
                                <TableCell colSpan={row.getVisibleCells().length}>
                                    ...
                                    {!expansionRenderContent && expansionComponent &&
                                        React.createElement(expansionComponent, { row: row.original })}
                                </TableCell>
                            </TableRow>
                        )}
                    </>
