...
function createZodSchema(schemaProperty?: JSONSchemaProperty, t?: Record<string, string>): z.ZodType {
    ...
    switch (schemaProperty.type) {
        case 'string':
            schema = z.string({
                message: t?.['validation.invalid_string'] ?? 'validation.invalid_string',
            });
            ...
            if (schemaProperty.minLength !== undefined) {
                schema = (schema as z.ZodString).min(
                    ...
                    t?.['validation.min_length']
                        ? (t['validation.min_length'] ?? '').replace('{min}', String(schemaProperty.minLength))
                        : undefined
                );
            }
            if (schemaProperty.maxLength !== undefined) {
                schema = (schema as z.ZodString).max(
                    ...
                    t?.['validation.max_length']
                        ? (t['validation.max_length'] ?? '').replace('{max}', String(schemaProperty.maxLength))
                        : undefined
                );
            }
        ...
        case 'number':
            schema = z.number({
                message: t?.['validation.invalid_number'] ?? 'validation.invalid_number',
            });
            ...
            if (schemaProperty.minimum !== undefined) {
                schema = (schema as z.ZodNumber).min(
                    ...
                    t?.['validation.min_value']
                        ? (t['validation.min_value'] ?? '').replace('{min}', String(schemaProperty.minimum))
                        : undefined
                );
            }
            if (schemaProperty.maximum !== undefined) {
                schema = (schema as z.ZodNumber).max(
                    ...
                    t?.['validation.max_value']
                        ? (t['validation.max_value'] ?? '').replace('{max}', String(schemaProperty.maximum))
                        : undefined
                );
            }
            break;
        ...
        case 'integer':
            schema = z.number({
                message: t?.['validation.invalid_integer'] ?? 'validation.invalid_integer',
        ...
        case 'boolean':
            schema = z.boolean({
                message: t?.['validation.invalid_boolean'] ?? 'validation.invalid_boolean',
            });
            break;
    ...
    if (schemaProperty.enum && Array.isArray(schemaProperty.enum)) {
        schema = z.enum(schemaProperty.enum as [string, ...string[]],
...
export function CellRenderer({
    ...
    const datePickerIdRef = useRef<string>(`date-${schemaProperty?.title || 'field'}-${Date.now()}`);
    ...
    const debouncedUpdate = useCallback((newValue: unknown) => {
        if (updateTimeoutRef.current) {
            clearTimeout(updateTimeoutRef.current);
        }
    ...
    if (editable && !schemaProperty?.readOnly) {
        if (fieldName && customRenderers?.byField?.[fieldName]) {
            ...
            return <>{customRenderer({ value: localValue, onUpdate: handleChange, error: validationError || undefined, schemaProperty, t })}</>;
        }

        if (schemaProperty?.format && customRenderers?.byFormat?.[schemaProperty.format]) {
        ...
        if (schemaProperty?.type === 'number' || schemaProperty?.type === 'integer') {
            const handleNumberChange = (e: React.ChangeEvent<HTMLInputElement>) => {
                const val = schemaProperty.type === 'integer'
        ...
        if (schemaProperty?.format === 'date' || schemaProperty?.format === 'date-time') {
            const dateValue = localValue instanceof Date
                ? localValue
                : localValue
            ...
            return (
                <ErrorWrapper error={validationError} mode={errorDisplayMode}>
                    <DatePicker
                        ...
                        showIcon={false}
                        defaultValue={dateValue instanceof Date && !Number.isNaN(dateValue.getTime()) ? dateValue : undefined}
                        onChange={handleDateChangeRef.current}
                        classNames={{
                            dateInput: cn('shadow-none border-0 h-8! rounded-none', validationError && 'border border-destructive')
                        }}
                        useTime={isDateTime}
                    />
                </ErrorWrapper>
